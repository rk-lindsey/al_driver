...
def cleanup_and_setup(*argv, **kwargs):
	...
	for i in args_targets: # 20 all
		...
		if os.path.isdir(dftb_dir):
			...
			dftb_dir = args["build_dir"] + "/DFTB-" + i + "/CASE-" + `args_this_case`
			...
			helpers.run_bash_cmnd("mkdir -p  " + dftb_dir + "/CASE-" + `args_this_case`)
		...
		if os.path.isdir("DFTB-" + args_targets[i] + "/" + CASE_PATH):
			...
			if count_com > count_log:
				...
				if args["job_system"] == "slurm":
					job_list.append(helpers.run_bash_cmnd("msub run_dftb.cmd").replace('\n', ''))
				else:	
					job_list.append(helpers.run_bash_cmnd("msub run_dftb.cmd").replace('\n', ''))
