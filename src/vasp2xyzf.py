...
#
# Note: Simulation temperature read based on the POSCAR comment line
#       ... Expects something like: "C N O ( 300 K)"
...
if False:
    ...
    BOXLENS.append(repr((abs(float(X)))))
    BOXLENS.append(repr((abs(float(Y)))))
    BOXLENS.append(repr((abs(float(Z)))))
...
print("Total atoms                      : " + repr(TOTAL_ATOMS)) 
print("Simulation temperature           : " + TEMPERAT + " (K)")
print("Simulation timestep              : " + TIMESTEP + " (fs)")
print("Frame skip set to                : " + repr(SKIP))
print("Time between generated xyz frames: " + repr(round((SKIP)*float(TIMESTEP),5)) + " (fs)")
...
for LINE in IFSTREAM:
    ...
    if START_READ == 1:        # Ignore current (title) line and the "----" line that follows
        if TOTAL_FRAMES%SKIP == 0: # Then we should print this frame
            ...
            TMSTREAM.write(repr(SIGMA) + '\n')

            # Print the xyz file headers
        
            OFSTREAM.write(repr(TOTAL_ATOMS) + '\n')
    ...
    if START_READ == 2 and READ_ATOMS < TOTAL_ATOMS:
        ...
        LINE[3] = repr(float(LINE[3])/1.8897161646320724/27.2114)
        LINE[4] = repr(float(LINE[4])/1.8897161646320724/27.2114)
        LINE[5] = repr(float(LINE[5])/1.8897161646320724/27.2114)
        LINE = ' '.join(LINE)
...
print("Counted total frames             : " + repr(TOTAL_FRAMES))
print("Printed total frames             : " + repr(PRINTED_FRAMES))
print("")
print("================================================================")
print("")        
