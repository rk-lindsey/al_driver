...
def cleanup_and_setup(*argv, **kwargs):
	...
	for i in args_targets: # 20 all
		...
		if os.path.isdir(gaus_dir):
			...
			gaus_dir = args["build_dir"] + "/GAUS-" + i + "/CASE-" + `args_this_case`
			...
			helpers.run_bash_cmnd("mkdir -p  " + gaus_dir + "/CASE-" + `args_this_case`)
...
def post_process(*argv, **kwargs):
	...
	for i in xrange(len(args_targets)): # 20 all
		...
		for j in xrange(args_cases):
			...
			outcar_list += sorted(glob.glob("CASE-" + `j` + "/*R.1*.log"))
...
def setup_gaus(my_ALC, *argv, **kwargs):
	...
	for i in xrange(len(args_targets)): # 20 all ... will always just be "all" for gaussian
